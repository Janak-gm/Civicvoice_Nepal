...
export default function CommunityFeedPage() {
  ...
  useEffect(() => {
    ...
    async function fetchFeed() {
      ...
      setError(null)
      try {
        const testNames = ['Final', 'Test5', 'Test3', 'Test User']
        let accumulated = null as FeedResponse | null
        ...
        while (true) {
          ...
          const visible = result.results.filter(r => {
            if (testNames.includes(r.citizen_name)) return false
            return r.images.length > 0 || r.videos.length > 0
          })

          if (!accumulated) {
            accumulated = { ...result, results: [...result.results] }
          } else {
            accumulated.results.push(...result.results)
            accumulated.next = result.next
          }

          if (visible.length > 0 || !result.next) break
          currentPage++
        }
