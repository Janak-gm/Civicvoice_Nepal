...
class CitizenLoginView(APIView):
    ...
    def post(self, request):
        ...
        if user.role != User.Role.CITIZEN:
            return Response(
                {"detail": "Only citizens can login here."},
                status=status.HTTP_403_FORBIDDEN,
            )
        if user.is_deleted:
            return Response({"detail": "Account is scheduled for deletion."}, status=status.HTTP_403_FORBIDDEN)
        refresh = RefreshToken.for_user(user)
        return Response(
            {
                "user": UserSerializer(user).data,
...
class OfficialLoginView(APIView):
    ...
    def post(self, request):
        ...
        if user.role != User.Role.OFFICIAL:
            return Response(
                {"detail": "Only officials can login here."},
                status=status.HTTP_403_FORBIDDEN,
            )
        if user.is_deleted:
            return Response({"detail": "Account is scheduled for deletion."}, status=status.HTTP_403_FORBIDDEN)
...
class LegacyLoginRedirectView(APIView):
    permission_classes = [AllowAny]

    def post(self, request):
        return Response(
            {
                "detail": "Use /api/accounts/login/citizen/ for citizens or /api/accounts/login/official/ for officials.",
                "citizen_login": "/api/accounts/login/citizen/",
                "official_login": "/api/accounts/login/official/",
            },
