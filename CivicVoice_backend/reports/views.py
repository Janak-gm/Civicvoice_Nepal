...
from ai.services import moderate_report
...
class ReportViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        elif self.action in ("status", "progress", "moderate", "pending_moderation", "remove"):
    ...
    def perform_create(self, serializer):
        report = serializer.save(citizen=self.request.user, visibility=False)
    ...
    def remove(self, request, pk=None):
        ...
        if not reason:
            return Response(
                {"detail": "A reason for removal is required."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        Notification.objects.create(
            ...
            message=f"Your report has been removed by an official.\n\nReason: {reason}",
        )
        ...
        return Response({"detail": "Report removed successfully."}, status=status.HTTP_200_OK)
