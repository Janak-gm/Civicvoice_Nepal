...
class SummaryView(APIView):
    ...
    def get(self, request):
        ...
        report_agg = Report.objects.filter(visibility=True).aggregate(
...
class RecentReportsView(ListAPIView):
    ...
    def get_queryset(self):
        qs = Report.objects.filter(visibility=True).select_related("citizen").only(
            "id",
            "title",
            "category",
            "status",
...
class AnalyticsView(APIView):
    ...
    def get(self, request):
        base_qs = Report.objects.filter(visibility=True)
